...
export type DashboardData = {
  ...
  activities: Array<{ id: number; postId: number | null; postTitle: string; action: string; description: string; actorEmail: string; createdAt: string }>;
};
...
export default function AdminDashboard({ displayName, initialData }: { displayName: string; initialData?: DashboardData }) {
  ...
  useEffect(() => {
    ...
    const empty: DashboardData = { stats: { posts: 0, idea: 0, published: 0, draft: 0, review: 0, approved: 0, scheduled: 0, seoIssues: 0, categories: 0, media: 0, users: 0 }, recent: [], analytics: [], opportunities: [], performance: { organic: emptyMetric, engagement: emptyMetric, framework: emptyMetric, leads: emptyMetric }, seo: { score: 0, metaIssues: 0, brokenLinks: 0, orphanPages: 0, missingAlt: 0 }, activities: [] };
  ...
  return <div className="op8-dashboard">
    ...
    <div className="op8-dashboard-grid">
      ...
      <section className="op8-panel"><h2>Son Hareketler</h2><div className="op8-activity">{(data?.activities ?? []).slice(0, 4).map((activity) => <div className="op8-activity-row" key={activity.id}><span title={new Date(activity.createdAt).toLocaleString("tr-TR")}>{relativeTime(activity.createdAt)}</span><strong>{activity.description}<small>{activity.actorEmail ? ` · ${activity.actorEmail}` : ""}</small></strong></div>)}{!data?.activities?.length && <div className="op8-activity-row"><span>—</span><strong>Henüz hareket bulunmuyor.</strong></div>}</div><a className="op8-view-link" href="/admin/yazilar">Tüm hareketler →</a></section>
...
function relativeTime(value: string) { const time = new Date(value).getTime(); if (!Number.isFinite(time)) return "—"; const minutes = Math.max(0, Math.floor((Date.now() - time) / 60000)); if (minutes < 1) return "Şimdi"; if (minutes < 60) return `${minutes} dk`; const hours = Math.floor(minutes / 60); if (hours < 24) return `${hours} sa`; const days = Math.floor(hours / 24); if (days < 7) return `${days} gün`; return new Date(value).toLocaleDateString("tr-TR"); }

...
export type SiteUser={userId:string;email:string;displayName:string;fullName:string;role:string;status:string};
...
export async function createUserSession(userId:number,remember=false){const token=randomBytes(32).toString("base64url"),expires=new Date(Date.now()+(remember?30*86400000:12*3600000));await getPool().execute("INSERT INTO user_sessions(user_id,token_hash,expires_at) VALUES(?,?,?)",[userId,digest(token),expires]);await getPool().execute("UPDATE users SET last_login_at=NOW() WHERE id=?",[userId]);(await cookies()).set(COOKIE_NAME,token,{httpOnly:true,secure:process.env.NODE_ENV==="production"&&process.env.LOCAL_TEST_MODE!=="1",sameSite:"lax",path:"/",expires})}

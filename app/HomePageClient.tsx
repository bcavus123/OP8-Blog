...
function Featured({section,posts}:{section:HomeSection;posts:PublishedPost[]}){const post=choosePosts(section,posts,1)[0];if(!post)return null;return <section className="featured shell"><div className="feature-art">{post.coverUrl?<img src={post.coverUrl} alt={post.coverAlt}/>:<><span className="feature-number">01</span><p>{post.categoryName||"YazÄ±"}<br/>SeÃ§kisi</p></>}</div><article className="feature-copy"><p className="kicker">{section.eyebrow} <span>{post.categoryName||"Genel"}</span></p><h2>{post.title}</h2><p>{post.excerpt}</p><div className="meta"><span>{publishedDate(post.publishedAt)}</span><span>{readingTime(post)} okuma</span></div><Link className="text-link" href={`/yazilar/${post.slug}`}>YazÄ±yÄ± oku <span>â†’</span></Link></article></section>}
function Latest({section,posts}:{section:HomeSection;posts:PublishedPost[]}){const chosen=choosePosts(section,posts,3);if(!chosen.length)return null;return <section className="posts shell" id="yazilar"><div className="section-head"><div><p className="eyebrow">{section.eyebrow}</p><h2>{section.title}</h2></div></div><div className="post-grid">{chosen.map((post,index)=><article className="post-card" key={post.id}><div className={`card-art ${["mint","lilac","peach"][index%3]}`}>{post.coverUrl?<img src={post.coverUrl} alt={post.coverAlt}/>:<><span>{String(index+1).padStart(2,"0")}</span><div className="shape"/></>}</div><div className="card-body"><p className="kicker">{post.categoryName||"Genel"}</p><h3>{post.title}</h3><p>{post.excerpt}</p><div className="meta"><span>{publishedDate(post.publishedAt)}</span><span>{readingTime(post)}</span></div><Link href={`/yazilar/${post.slug}`}>Okumaya baÅŸla â†’</Link></div></article>)}</div></section>}
function Topics({section,topics}:{section:HomeSection;topics:Topic[]}){if(!topics.length)return null;return <section className="topics shell" id="konular"><p className="eyebrow">{section.eyebrow}</p><h2>{section.title}</h2><div className="topic-list">{topics.map(topic=><a href="#yazilar" key={topic.id}>{topic.name} <span>{topic.postCount} yazÄ±</span></a>)}</div></section>}
function ContentSection({section}:{section:HomeSection}){return <section className="custom-home-section shell"><div>{section.imageUrl&&<img src={section.imageUrl} alt={section.imageAlt}/>}</div><article><p className="eyebrow">{section.eyebrow}</p><h2>{section.title}</h2><p>{section.body}</p>{section.buttonLabel&&<a className="button" href={section.buttonHref}>{section.buttonLabel} â†’</a>}</article></section>}
...
export default function HomePageClient({initialConfig,initialPosts,initialTopics}:{initialConfig:HomeConfig;initialPosts:PublishedPost[];initialTopics:Topic[]}){
  ...
  return <main style={themeStyle(config)}>
    ...
    <section className={`hero shell ${config.hero.imageUrl?"hero-has-image":""}`}>
      ...
      <div className="hero-free-layout"><p className="eyebrow" style={{...config.hero.eyebrowStyle,...boxStyle(config.hero.layout.eyebrow)}}>{config.hero.eyebrow}</p><h1 style={boxStyle(config.hero.layout.title)}><span style={config.hero.titleStyle}>{config.hero.title}</span></h1><em className="hero-free-accent" style={{...config.hero.accentStyle,...boxStyle(config.hero.layout.accent)}}>{config.hero.accent}</em><p className="hero-copy" style={{...config.hero.descriptionStyle,...boxStyle(config.hero.layout.description)}}>{config.hero.description}</p><a className="button hero-free-button" style={boxStyle(config.hero.layout.button)} href={config.hero.buttonHref}>{config.hero.buttonLabel} <span>â†“</span></a></div>
    ...
    {loaded&&!posts.length&&<section className="shell empty-state" id="yazilar"><h2>Ä°lk yazÄ±lar hazÄ±rlanÄ±yor.</h2><p>YayÄ±mlanan iÃ§erikler otomatik olarak burada gÃ¶rÃ¼necek.</p></section>}
    ...
    <footer id="hakkinda"><div className="shell footer-inner"><div><Brand config={config} footer/><p>OP8 deÄŸer yaratma yaklaÅŸÄ±mÄ±na iliÅŸkin analizler ve uygulama notlarÄ±.</p></div><div>{config.headerMenu.filter(x=>x.visible).map(x=><a href={x.href} key={x.id}>{x.label}</a>)}</div><p>Â© 2026 OP8 Operating Partner Value Creation Framework</p></div></footer>
  </main>
}

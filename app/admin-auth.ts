...
export async function getActor():Promise<Actor|null>{if(process.env.LOCAL_TEST_MODE==="1")return{userId:"preview",email:"preview@op8.local",displayName:"OP8 Admin",fullName:"OP8 Admin",role:"super_admin",status:"active",bootstrap:true};const user=await getCurrentUser();if(user)return{...user,role:(user.role||"member")as Role};return basicActor()}
